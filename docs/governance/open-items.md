...
| **OQ-5** | Whether the inline ADR-001 to ADR-012 in A3 are authoritative as part of an authoritative document. `70-adr.md` §70.1 states the reading; the question is open, and the two ADR numbering spaces must never be conflated | — | An editorial confirmation by the architecture owner |
