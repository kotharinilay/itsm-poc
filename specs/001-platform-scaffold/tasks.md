...
- [X] T128 Implement the model adapter in `ragcore/src/ragcore/model/` behind the port from T027, routed exclusively through the AI Gateway — Boundary: model | Validates: Spec §FR-OPS-007
...
- [X] T136 [P] Write adapter tests in `ragcore/tests/egress/test_adapters.py` exercising each adapter against a fake honouring its contract — Boundary: integration | Validates: Constitution §Required test categories
- [X] T137 [P] Write a test in `ragcore/tests/egress/test_no_provider_leak.py` asserting no provider SDK or model type appears in `domain/` or `application/` — Boundary: layering | Validates: Spec §FR-EXT-011
- [X] T138 [P] Write a test in `ragcore/tests/architecture/test_no_direct_model_call.py` asserting no module holds a provider endpoint outside `model/` — Boundary: model egress | Validates: Constitution §Model access
...
- [X] T234 [P] Emit the **customer** audience OpenAPI document from the running services — FastAPI for RagCore, the built-in generator for .NET Minimal APIs — written to `build/contracts/customer.v1.openapi.json` *(delivered at `build/contracts/ragcore/customer.v1.openapi.json` and `build/contracts/dotnet/customer.v1.openapi.json` — the tree gained a per-deployable level once a second and third deployable emitted documents; path amended 2026-09-18, task unchanged)* — Boundary: API contract | Validates: Spec §FR-DEMO-013
- [X] T235 [P] Emit the **staff** audience OpenAPI document to `build/contracts/staff.v1.openapi.json` *(delivered at `build/contracts/ragcore/staff.v1.openapi.json` and `build/contracts/dotnet/staff.v1.openapi.json`; path amended 2026-09-18, task unchanged)* — Boundary: API contract | Validates: Spec §FR-DEMO-013
- [X] T236 [P] Emit the **workload** audience OpenAPI document to `build/contracts/workload.v1.openapi.json` *(delivered at `build/contracts/ragcore/workload.v1.openapi.json` and `build/contracts/integrations/workload.v1.openapi.json`; path amended 2026-09-18, task unchanged)*. **One document per audience; a merged document is prohibited** — it would let a customer-facing client discover the staff and workload surfaces — Boundary: API contract | Validates: Contracts §README rule 5
...
- [X] T174 [US1] Implement content safety in `ragcore/src/ragcore/model/safety.py` — inbound before the model, outbound before a response returns, every decision logged with its correlation identifier — Boundary: model egress | Validates: Spec §FR-AGENT-010, §FR-AGENT-013
...
- [X] T296 **Remove** `ragcore/src/ragcore/integrations/` **except `model/`**, together with its connector settings and secret references in `ragcore/src/ragcore/config/` and its connector dependencies in `ragcore/pyproject.toml` and `uv.lock`. `model/` **stays**: model egress and content safety are reasoning, not integration — Boundary: Agent/RagCore | Validates: Spec §FR-INTEG-016, ADR-0007
- [X] T297 Write an architecture test in `ragcore/tests/architecture/test_no_connector_in_ragcore.py` asserting **no adapter, connector, MCP client or external-provider client remains in RagCore**, and that no module outside `model/` holds an external endpoint — Boundary: layering | Validates: Spec §FR-DEMO-021, §FR-EXT-011
...
> **X9**, **X11**, **X12**.
>
> **X5 is CLOSED — 2026-09-18.** `ragcore/tests/security/test_integration_grants.py` exists and
> references `synthia_integrations` directly, asserting that a write to `catalogue_id`,
> `catalogue_version`, `parameters` or `tenant_id` on `integration_job` is refused **by PostgreSQL**
> — which is exactly what T283 specified and what `FR-INTEG-020` rests on. The test lives in
> `ragcore/tests/` rather than the `integrations/tests/security/test_job_grants.py` path T283 named,
> because the migrations that create the grant are RagCore's; T283 is correctly `[X]` and the path
> in its description is the original, not the delivered one. **X5 requires no work.**
>
...
| T138 | No-direct-model-call test | Guards `model/`, which stays |
...
**Appended by `/speckit-converge` on 2026-09-18.** These are gaps between the artefacts and the
codebase that no existing task covers. They do **not** reopen anything retired: ADR-0008's re-entry
condition forbids the deferred certificate mechanism returning to the backlog without a superseding
decision, so none of the tasks below implements a gateway-to-backend provenance control, and none
may be completed by introducing one.

- [X] T328 **CRITICAL** — Amend `.specify/memory/constitution.md` to record the accepted deviation from **Principle I**, which still states unconditionally that "No application API is reachable by a path that bypasses the gateway" and that "A client-supplied tenant, role or audience MUST NEVER establish authority". Since [ADR-0008](../../docs/adr/0008-defer-certificate-based-gateway-to-backend-provenance.md) deferred gateway-to-backend provenance with no replacement, a caller inside the container apps environment can reach a backend directly and assert any tenant and role — and the constitution contains **no mention of ADR-0008 at all**. Its own amendment procedure governs: *"Silent divergence is a defect: practice that has drifted is corrected in the practice or amended here, never left undocumented."* Follow that procedure — name the principle affected, give the rationale, describe the migration path, cite ADR-0008, update the Sync Impact Report and the Last Amended date. Per §Versioning policy this is a **MAJOR** bump (3.2.0 → 4.0.0): "relaxing a non-negotiable". **Do not satisfy this task by implementing a replacement control** — Boundary: governance | Validates: Constitution I, Constitution §Amendment procedure, Principle X (contradicts)
- [X] T329 **CRITICAL** — Reconcile **Principle VIII and §Coverage** with the same deferral, in the same amendment as T328 or immediately after it. §Coverage requires that "every protection named as a hard failure in Principle VIII must have a test that fails when the protection is removed", and Principle VIII names *"tenant context derived from an untrusted client field"* and *"authorization bypass"* among those hard failures. The tests that discharged that requirement — `ragcore/tests/security/test_gateway_provenance.py`, `dotnet/tests/Synthia.ContractTests/GatewayProvenanceTests.cs` and the self-supplied-contract case in `integrations/tests/security/test_boundary.py` — were deleted because the protection itself was removed, so no such test can now exist or pass. Either record the exemption explicitly against those two hard failures or narrow the hard-failure list, and say which. Leaving §Coverage as written makes it an unsatisfiable gate that a future reviewer will read as met — Boundary: governance | Validates: Constitution VIII, Constitution §Coverage (contradicts)
- [X] T330 Correct two stale records in this file, neither covered by T321's enumerated list. First, the **X5** entry in the 2026-09-18 progress note still reads "no test anywhere references the `synthia_integrations` database principal, so the column-scoped grant behind `FR-INTEG-020` remains unexercised — this is T283 and it is still the top item"; T283 is `[X]` and `ragcore/tests/security/test_integration_grants.py` asserts exactly the refusal of `catalogue_id`, `catalogue_version`, `parameters` and `tenant_id` on `integration_job`, so X5 is closed and should be marked so the way X7 already is. Second, **T234–T236** name `build/contracts/{customer,staff,workload}.v1.openapi.json`, but the committed layout is per-deployable — `build/contracts/{ragcore,dotnet,integrations}/…`. Amend the records; do not renumber or reopen the tasks — Boundary: documentation | Validates: Constitution X (partial)

---

## Phase 19: Verification remediation *(2026-09-19)*

**A verification pass ran every gate, started every service natively, and built and ran every
container image.** The gates were almost entirely green and the architecture held — no cross-tree
dependency, no token parsed, the monolith read-only, the database grants refused by PostgreSQL, no
role implying another. What it found was that **the platform did not run as documented**: no image
could be built or started, the committed manifests configured a setting RagCore does not read, and
several gates passed by never running.

**No task is reopened and no id is reused.** Each item below names the `[X]` tasks whose claim it
corrects; those keep their marks, because the work they describe was done and what follows is the
gap between it and what a running system needed.

### Delivered

- [X] T331 Make every container image build, start and stay hardened. `dotnet.Dockerfile` restored a solution whose test projects `.dockerignore` excludes (**T146**); both Python images built the virtual environment at `/build` and ran it from `/app`, so console-script shebangs named a missing interpreter and neither `uvicorn` nor `alembic` could execute (**T147**, **T269**, **T149**); the placeholder digest in `FROM` failed BuildKit's parse for every stage, so even the "build stage only" CI job could not run (**T274**). The runtime base is now an `ARG` defaulting to the placeholder — a committed build is still refused — and `build/scripts/smoke-images.sh` builds with a labelled CI-only tag override, starts each image read-only with the manifests' own variable names, and asserts non-root, no package manager and a working `alembic`. `.github/workflows/images.yml` runs it; `dev.sh images` is the same gate locally. `base-image-digests.yml` now re-pins the Integrations image and scans both base families (**T148a**) — Boundary: containers | Validates: Constitution §Containers
- [X] T332 Make the committed configuration start the process. `Settings` accepted only `SYNTHIA_DATABASE__DSN`, while `ragcore.yaml`, `migrate.job.yaml`, `env.py` and `provision_checkpoint_schema.py` all use `SYNTHIA_DB_DSN` — so a replica configured exactly as committed failed validation, and the migration job's checkpoint step failed after Alembic had already applied (**T069**, **T084**, **T149**). Every settings group is built by a factory at construction rather than an instance at import. `tests/unit/test_settings.py` asserts every `SYNTHIA_*` name in both manifests is one the settings read — Boundary: configuration | Validates: research R-019, quickstart V17
- [X] T333 Derive the checkpoint thread from trusted identity, and prune the key that is actually written. `thread_config` used the caller-supplied session identifier alone, so before the triage gate — when by design no `chat_session` row exists (`FR-SESS-003`) — nothing bound a conversation to its owner (**T176**). `ragcore/src/ragcore/graph/threads.py` derives it from organisation, requester and session. The retention sweeper pruned work-item identifiers, which no thread was ever written under, so no conversation's checkpoints were ever removed and the tests asserted the wrong key (**T106**) — Boundary: orchestration, retention | Validates: Constitution P-IV, Spec §FR-SESS-020
- [X] T334 Authenticate RagCore's synchronous call to the Integrations Service. The client sent a correlation header and no token, while `workload.v1.xml` runs `validate-azure-ad-token` and requires the Integrations application role — every call would have been refused at the gateway (**T305**, **T306**, **T272**, **T273**). It now presents RagCore's own app-only token from the shared managed identity, for a required `SYNTHIA_INTEGRATIONS_ENTRA_SCOPE`. First tests for this client — Boundary: service boundary | Validates: Spec §FR-INTEG-011, Plan §Authentication
- [X] T335 One correlation-identifier rule across all three deployables, in `build/policy/correlation-id.json`. RagCore accepted any printable string and echoed `<script>` into a header and a problem body; the Integrations Service accepted hex only, on the HTTP hop **and on the command envelope**, so a command carrying an identifier RagCore had accepted was dead-lettered and authorized work never ran (**T058**, **T071**, **T261**). Each stack asserts its own implementation against the shared vectors. RagCore's correlation middleware now owns the response header, which also removes a duplicate `X-Correlation-Id` — Boundary: observability | Validates: Spec §FR-OPS-001, Contracts §README
- [X] T336 Refuse a client-supplied authority parameter at the Integrations Service, as RagCore and the monolith do. It was silently ignored: never honoured, but indistinguishable from an attempt nobody made (**T263**) — Boundary: identity | Validates: Constitution P-I
- [X] T337 Compare emitted contracts against the contracts as committed. `contracts.yml` copied them aside before the run and then compared `build/contracts`, which the .NET emitter overwrites during the job — so the .NET documents were compared with themselves and the gate passed whatever they said (**T238**, **T239d**) — Boundary: CI | Validates: Spec §SC-DEMO-011
- [X] T338 Run the gates that existed but never ran. The accessibility sweep was conditional on a `test:a11y` script that did not exist, so CI printed a notice instead of sweeping (**T016**, **T049**); `npm test` covered four of seven Angular projects, leaving `customer-features`, `staff-features`, `staff-portal` and the no-authz architecture check in no gate at all (**T047**) — Boundary: CI | Validates: Spec §SC-SURF-002, §FR-SURF-004
- [X] T339 Make `dev.sh validate` true to its own claim. It ran a strict subset of CI while the README said otherwise: desktop lint, build and the security-guard prover, the web type check and sweep, the contract gates and the edge checks were pipeline-only. `images` is a separate command because it needs Docker — Boundary: tooling | Validates: Constitution §Quality gate
- [X] T340 Assert the two banned APIs that were analyzer-only. `.Result` and a general catch that does not rethrow are now architecture tests (**T034**), which is what the task asked for: an analyzer can be suppressed at a call site with a pragma, and a test cannot be suppressed without deleting it. Proven by planting both with the analyzer suppressed — Boundary: .NET | Validates: Constitution §.NET baseline
- [X] T341 Assert the tenant-isolation contract through the real pipeline. The suite checked that a filter predicate existed and that view rows carry an organisation; nothing exercised a read path or the 404 a foreign resource must return (**T161**). `TenantIsolationContractTests` drives identity → scope → query → status, with the store stood in for and that limit stated in the file — Boundary: tenant isolation | Validates: Spec §SC-IDENT-002
- [X] T342 Remove 62 `# noqa` codes naming rules the Ruff selection does not enable. Each suppressed nothing; the explanations remain as ordinary comments — Boundary: Python | Validates: Constitution §Python baseline
- [X] T343 Correct the runbook. `README.md` claimed a gate command that cannot work (`npm run test --workspace apps/desktop`, in a repository with no root `package.json`), documented no way to start any service, named no environment variable and described no Docker path; `quickstart.md` gave four commands that fail as written — `alembic upgrade head` (not on `PATH`), `uvicorn ragcore.api:app` (the app is a factory), `npm run start:customer-portal` (no such script) and a worker whose `main()` raises by design. Both now carry a runnable local path, and the README gate table names the working directory each command runs from. Covers part of **T321**; its remaining items stand — Boundary: documentation | Validates: Constitution P-X

### Open

- [X] T344 Define the **hosting tier for the two web portals**, and deliver the CSP from it ([ADR-0009](../../docs/adr/0009-portal-delivery-one-origin-per-browser-surface.md)). Each surface gets its **own Front Door endpoint, origin group and container app** — separate origins, because the browser's security boundary is the origin and a shared one would share storage, cookies and any injected script between a customer session and a staff session. Both reach their container over Private Link with internal-only ingress, under **one** WAF policy. `build/docker/portals.Dockerfile` serves the bundle and sets the Stage 3 baseline with a per-response nonce — the same `csp-host.mjs` the CSP suite drives, importing `buildCspHeader` rather than restating it, because a static host cannot mint a nonce (**T048**, **T346**). The edge guard's "exactly one origin" rule becomes an allow-list by name, plus a new rule that **only the gateway origin may serve `/api`**; both halves are proven by planted violations in `verify-edge-guard.sh`, and the image gate asserts each portal image serves a nonced policy with no weakening token. **What is still not provisioned** is the environment itself: host names, DNS and the CORS/`connect-src` reconciliation land with T227a — Boundary: browser security | Validates: Constitution P-VII, Plan Stage 3
- [X] T346 Send the CSP baseline and fix what that exposed. The policy had been built and unit-tested but **never delivered**, and served for the first time against the production build it blocked the application: `base-uri 'none'` forbids the `<base href="/">` every Angular document carries, on both portals and the desktop renderer. `base-uri` is now `'self'` on every surface, with the deviation and the evidence recorded in plan Stage 3 and the Stage 3 checklist — including why deleting the tag costs more (relative asset URLs make a deep link 404) and why `'self'` still refuses the attack that matters, a `<base>` pointing at another origin. **One value on every surface and in every environment**: a directive that is strict in production and relaxed locally is one nobody exercises before it ships — Boundary: browser security | Validates: Constitution P-VII, Plan Stage 3
- [ ] T345 Implement **audit retention and per-organisation audit erasure**. `T106` requires audit events to be removed after seven years and `T107` requires an organisation's records to be erased, but no principal holds `DELETE` on `platform.audit_event`: the runtime principal deliberately holds `SELECT, INSERT` (revision 0019) and `erasure.py` and `retention_sweep.py` both name audit as out of their reach, deferring it to "a privileged, separately-audited job" that does not exist. **The absence is correct; the missing piece is the job.** It needs its own principal with `DELETE` and nothing else, a worker, and grant tests against a real PostgreSQL — Boundary: retention, Audit | Validates: Spec §FR-AUDIT-004, §FR-AUDIT-005, §FR-AUDIT-006

> **Recorded, not fixed, and deliberately so.** The verification pass also observed that a
> well-formed self-supplied `X-Idp-*` contract presented straight to a backend is honoured. That is
> [ADR-0008](../../docs/adr/0008-defer-certificate-based-gateway-to-backend-provenance.md)'s stated
> and accepted consequence (constitution §Recorded deviations, D-01), not a new finding, and nothing
> above introduces a replacement control — ADR-0008 must be superseded first.
