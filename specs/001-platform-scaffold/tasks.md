...
- [ ] T344 [P] Define the **production hosting tier for the two web portals**. The contract it must satisfy is now executable rather than described: `apps/web/scripts/csp-host.mjs` is a reference tier — a fresh nonce per document, the same nonce on `ngCspNonce`, and the header from `buildCspHeader` itself rather than a restatement — and `e2e/csp-hosting.spec.ts` drives the built portals through it, failing on any violation the browser reports (`npm run test:csp`, in CI and in `dev.sh validate`). **What remains is the deployed tier**: nothing in `build/infra/` serves the portals, and the reference host is not it (**T048**) — Boundary: browser security | Validates: Constitution P-VII, Plan Stage 3
