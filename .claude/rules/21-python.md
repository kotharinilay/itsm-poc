...
`applied_when: language == python`. **Nothing in this file is evidence of a .NET, TypeScript,
...
*Source: `python.yaml#baseline` · Enforcement: mechanical — both projects declare
`requires-python = ">=3.12,<3.13"` and `target-version = "py312"`*

### Toolchain
These six entries carry **no `id:` field in the source**; their identifiers are the Phase 2 matrix
row ids, preserved here (`docs/migration/phase-9-baseline-coverage.md` §2).

| Source | Requirement | Emitted to | Enforcement |
|---|---|---|---|
| `python.yaml#toolchain.linter` | Ruff. `[tool.ruff.lint] select` is written **explicitly** — the Ruff default is only `[E4, E7, E9, F]`, so the set must be stated | `pyproject.toml` | mechanical |
| `python.yaml#toolchain.formatter` | `ruff format`, `quote-style = "double"` | `pyproject.toml` | mechanical (see note) |
| `python.yaml#toolchain.import_sort` | Ruff `I` (isort), enabled by inclusion in `select`. **No standalone isort.** | `pyproject.toml` | mechanical |
| `python.yaml#toolchain.type_checker` | `mypy --strict` **or** `pyright` (`typeCheckingMode = "strict"`), run as a **CI gate** | `pyproject.toml` | mechanical |
| `python.yaml#toolchain.security` | Ruff `S` (flake8-bandit). **No separate Bandit run needed.** | `pyproject.toml` | mechanical |
| `python.yaml#toolchain.packaging` | PEP 621 `[project]` metadata in `pyproject.toml`, plus a **committed pinned lockfile** (`uv.lock` \| `poetry.lock` \| `requirements*.txt`) | `pyproject.toml` | mechanical — both projects use `uv` and commit `uv.lock` |
...
> it. The setting is nonetheless not written where the source says to emit it — recorded as **DV-8**
> (cosmetic) in `docs/migration/phase-9-baseline-coverage.md` §8.

### The three strictness switches

| # | Source | Switch | Realizes | Enforcement |
|---|---|---|---|---|
| **S1** | `python.yaml#P-PY-S1` | `[tool.mypy] strict = true` (or pyright strict) | PY-3 | mechanical — both projects set `strict = true` and add `warn_unreachable`, `warn_no_return`, `disallow_any_unimported`, `disallow_untyped_defs`, `no_implicit_reexport` |
| **S2** | `python.yaml#P-PY-S2` | `[tool.ruff.lint] select` includes `S, ASYNC, B, DTZ, PTH` (+ `E, F, I, UP, SIM`) | PY-22 | mechanical |
| **S3** | `python.yaml#P-PY-S3` | `ruff check` **and** `ruff format --check` run as **failing CI steps** | — | mechanical |
...
*Source: the three `strictness` entries of `python.yaml`, identified in the table above.*
...
*Source: `lang/python#PY22` · core · Enforcement: mechanical*
...
*Source: `python.yaml#P-PY-S3` · Enforcement: mechanical for `ruff check` failing CI; the
...
*Source: `lang/python#PY3` · satisfies `principles#P20` · Enforcement: mechanical — `mypy --strict`
...
syntax.
*Source: `lang/python#PY21`, `python.yaml#P-PY-3` · Enforcement: mechanical — Ruff `UP006`,
`UP007`/`UP045`, `UP035`*
...
invocations.
*Source: `lang/python#PY1` · core · satisfies `principles#P25` · Enforcement: mechanical — Ruff
`B006`, `B008`*
...
*Source: `lang/python#PY13` · Enforcement: mechanical — `B` is in `select` in both projects*
...
*Source: `lang/python#PY12` · core · satisfies `principles#P8` · Enforcement: mechanical — Ruff
`F401`, `F841`, `F821`*
...
*Source: `lang/python#PY20` · Enforcement: mechanical — Ruff `E711`*
...
*Source: `lang/python#PY24` · Enforcement: mechanical — Ruff `DTZ` family (`DTZ005`, `DTZ003`,
`DTZ002`)*
...
*Source: `lang/python#PY7` · Enforcement: mechanical — Ruff `PTH` family (`PTH118`, `PTH123`)*
...
*Source: `lang/python#PY2` · core · satisfies `anti-patterns#A15`, `principles#P30` · Enforcement:
...
*Source: `lang/python#PY14` · core · satisfies `anti-patterns#A30`, `principles#P30` · Enforcement:
...
*Source: `lang/python#PY6` · core · Enforcement: mechanical — Ruff `S101`, with the test-path split*
...
*Source: `lang/python#PY9` · satisfies `principles#P31` · Enforcement: mechanical — Ruff `SIM115`*
...
*Source: `lang/python#PY23` · core · satisfies `performance#PF1`, `anti-patterns#A31` ·
...
*Source: `lang/python#PY8` · core · satisfies `secure-coding#SC6` · Enforcement: mechanical — Ruff
`S307`, `S102`*
...
*Source: `lang/python#PY10` · core · satisfies `secure-coding#SC3` · Enforcement: mechanical — Ruff
`S602`, `S603`, `S607`*
...
*Source: `lang/python#PY25` · core · satisfies `secure-coding#SC4`, `anti-patterns#A25` ·
...
objects (RCE).
*Source: `lang/python#PY16` · core · satisfies `secure-coding#SC6` · Enforcement: mechanical — Ruff
`S301`, `S302`, `S506`*
...
*Source: `lang/python#PY15` · core · satisfies `anti-patterns#A24`, `secure-coding#SC18`,
...
*Source: `lang/python#PY18` · core · satisfies `secure-coding#SC22` · Enforcement: mechanical — Ruff
`S324`*
...
*Source: `lang/python#PY17` · core · satisfies `performance#PF6` · Enforcement: mechanical — Ruff
`S113`*
...
*Source: `lang/python#PY19` · Enforcement: mechanical — Ruff `S306`, `S108`*
...
*Source: `lang/python#PY4` · satisfies `principles#P29` · Enforcement: **currently-unenforced***
...
> Phase 9 records this and **does not** change the configuration
> (`.claude/rules/00-authority.md` §00.7). The rule is binding regardless: do not write `print` in
...
*Source: `lang/python#PY5` · Enforcement: mechanical — Ruff `UP032`, `UP031`*
...
throughout.
*Source: `lang/python#PY11`, `python.yaml#P-PY-1` · Enforcement: mechanical — Ruff `E` family and
...
Python test layout and naming (`python.yaml#P-PY-2`) is in `.claude/rules/40-testing.md` §40.3. It
