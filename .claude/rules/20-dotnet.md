...
**Nothing in this file is evidence of a Python,
...
*Origin: `dotnet#baseline` · Enforcement: mechanical (`Directory.Build.props`
...
|---|---|---|---|---|
| **S1** | `dotnet#P-DN-S1` | `<Nullable>enable</Nullable>` | DN-6 | mechanical — set |
| **S2** | `dotnet#P-DN-S2` | `<TreatWarningsAsErrors>true</TreatWarningsAsErrors>` | DN-7 | mechanical — set, plus `<CodeAnalysisTreatWarningsAsErrors>true</CodeAnalysisTreatWarningsAsErrors>` |
| **S3** | `dotnet#P-DN-S3` | `<EnableNETAnalyzers>true</EnableNETAnalyzers>` + `<AnalysisLevel>latest-Recommended</AnalysisLevel>` | DN-23 | mechanical — set |
| **S4** | `dotnet#P-DN-S4` | `<EnforceCodeStyleInBuild>true</EnforceCodeStyleInBuild>` | (see note) | mechanical — set |
| **S5** | `dotnet#P-DN-S5` | `<Deterministic>true</Deterministic>` + `<ContinuousIntegrationBuild>true</ContinuousIntegrationBuild>` (CI only) | — (reproducible builds) | mechanical — set, CI-conditioned |

> **Recorded defect in the retired input — D-1.** The input attributed `P-DN-S4`
> (`EnforceCodeStyleInBuild`) to the `goto` rule. `EnforceCodeStyleInBuild` is what makes the
> IDExxxx style rules build-failing, i.e. **DN-28 and DN-29**. Both DN-19 and DN-28/DN-29 are
> stated in full below, so no requirement is lost either way. Registered in
> `docs/governance/open-items.md` §4.

### Toolchain
These five entries carried no identifier of their own in the retired input; the identifiers below
are this file's.

| Origin | Requirement | Emitted to | Enforcement |
|---|---|---|---|
| `dotnet#toolchain.analyzers` | `EnableNETAnalyzers=true`, `AnalysisLevel=latest-Recommended` | `Directory.Build.props` | mechanical |
| `dotnet#toolchain.formatter` | `dotnet format`, driven by a committed `.editorconfig` | `.editorconfig` | mechanical (`dotnet format --verify-no-changes` in `.github/workflows/dotnet.yml`) |
| `dotnet#toolchain.style_in_build` | `EnforceCodeStyleInBuild=true` | `Directory.Build.props` | mechanical |
| `dotnet#toolchain.warnings` | `TreatWarningsAsErrors=true`; `CodeAnalysisTreatWarningsAsErrors` is **not** set false, so CA findings block the build too | `Directory.Build.props` | mechanical |
| `dotnet#toolchain.packages` | `ManagePackageVersionsCentrally=true` — `Directory.Packages.props` is the single source of package versions | `Directory.Packages.props` | mechanical (also `CentralPackageTransitivePinningEnabled=true`) |
...
*Origin: `lang/dotnet#DN6` · core · Enforcement: mechanical*
...
*Origin: `lang/dotnet#DN7` · core · Enforcement: mechanical*
...
> **DV-7** in `docs/governance/open-items.md` §1: it is a project-wide suppression of
...
*Origin: `lang/dotnet#DN23` · core · Enforcement: mechanical*
...
*Origin: `BL-01` · Enforcement: mechanical (`ModuleIsolationTests`, `BoundaryOwnershipTests`,
...
*Origin: `BL-02`; realizes `principles#P26` · Enforcement: mechanical for the service-locator ban
...
*Origin: `BL-03` · Enforcement: mechanical*
...
domain.
*Origin: `BL-35`; realizes `principles#P5`, `principles#P19` · Enforcement: partial (layering suites
...
*Origin: `BL-36`; realizes `principles#P8` · Enforcement: procedural*
...
*Origin: `BL-04` · Enforcement: procedural*

### BL-5 — API versioning
**URI segment versioning** — the version is a path segment: `/api/v1/...`.
*Origin: `BL-05` · Enforcement: partial (`ApimRoutingTests`)*
...
*Origin: `BL-06` · Enforcement: mechanical (`.github/workflows/contracts.yml`)*
...
*Origin: `BL-07` · Enforcement: partial (contract workflow diffs the emitted OpenAPI; the naming
...
*Origin: `BL-08` · Enforcement: procedural*
> **Recorded defect in the retired input — D-2.** The input's line for offset/limit pagination was
> truncated mid-sentence and ran into the next heading. What offset/limit pagination is permitted
> for — and whether it is permitted at all — **could not be read**, and was not reconstructed.
> Registered in `docs/governance/open-items.md` §4; decision **UD-4**. Everything legible is stated
> in full above.

### BL-18 — Input validation
**Use the .NET 10 built-in validation.** Do not add a third-party validation framework.
*Origin: `BL-18`; realizes `principles#P11`, `principles#P20` · Enforcement: procedural*
...
*Origin: `BL-19`; realizes `principles#P30` · Enforcement: partial*
...
**EF Core 10 + Npgsql.**
*Origin: `BL-09` · Enforcement: mechanical (`Directory.Packages.props` pins the versions centrally;
...
> **A retired input once attributed this to EF Migrations**, contradicting
> `.claude/rules/50-database.md` §50.7 and A2 §8.1. A human withdrew that attribution while keeping
> the requirement it carried — one versioned mechanism, gated in CI/deploy, never at startup. **The
> rule in force never changed**: it was `50-database.md` before and it is `50-database.md` now. Do
> not reintroduce EF Core migrations, a second migration tool, a second migration directory, or
> startup-time DDL.
...
*Origin: `BL-10` · Enforcement: mechanical — `NoMigrationTests` on the .NET
...
*Origin: `BL-11` · Enforcement: procedural*
...
*Origin: `BL-12` · Enforcement: procedural*
...
*Origin: `BL-13` · Enforcement: partial*
...
*Origin: `BL-14`; (2) realizes `secure-coding#SC4` via DN-24 · Enforcement: partial
...
*Origin: `lang/dotnet#DN24` · core · satisfies `secure-coding#SC4`, `anti-patterns#A25` ·
...
*Origin: `BL-16`; realizes `principles#P27` · Enforcement: procedural*
...
banned.**
*Origin: `BL-17`, `lang/dotnet#DN14` · Enforcement: see DN-14*
...
*Origin: `lang/dotnet#DN14` · core · satisfies `anti-patterns#A14` · Enforcement:
...
*Origin: `BL-15`; realizes `principles#P27`, `principles#P17` · Enforcement: partial
...
*Origin: `BL-20`, `lang/dotnet#DN3`; realizes `principles#P29` · Enforcement: mechanical via
...
*Origin: `lang/dotnet#DN30` · satisfies `principles#P29` · Enforcement: partial — CA2254 is on by
...
*Origin: `lang/dotnet#DN32` · satisfies `principles#P29` · Enforcement: partial — CA1727 is off by
...
*Origin: `BL-22`, `lang/dotnet#DN31` · satisfies `performance#PF11` · Enforcement:
...
`TreatWarningsAsErrors`.** Registered as **DV-2** in `docs/governance/open-items.md` §1.*

### BL-21 — Telemetry
**OpenTelemetry + the Azure Monitor Distro.**
*Origin: `BL-21` · Enforcement: procedural*
...
propagation headers.**
*Origin: `BL-24` · Enforcement: procedural*
...
*Origin: `BL-27` · Enforcement: partial*
...
*Origin: `BL-25` · Enforcement: procedural*
...
*Origin: `BL-26` · Enforcement: partial (`EdgeTrustPolicyTests`)*
...
*Origin: `BL-23`, `lang/dotnet#DN9` · core · Enforcement: mechanical —
...
captured context.
*Origin: `lang/dotnet#DN4` · core · satisfies `performance#PF1` · Enforcement: mechanical —
...
*Origin: `lang/dotnet#DN2` · core · satisfies `performance#PF1` · Enforcement: mechanical
...
*Origin: `lang/dotnet#DN5` · Enforcement: **currently-unenforced** — the source's mechanism is
...
ambient context).
*Origin: `lang/dotnet#DN8` · Enforcement: **currently-unenforced** — `dotnet/.editorconfig:84` sets
...
> it, which is itself an ADR trigger under `.claude/rules/70-adr.md` §70.2 B(1). It is **recorded**
> and the configuration is **not** changed (`.claude/rules/00-authority.md` §00.7). The baseline
...
application.
*Origin: `BL-30` · Enforcement: procedural · see also `.claude/rules/80-security-ops.md` §80.5 for
the runtime half*
...
*Origin: `lang/dotnet#DN27` · satisfies `anti-patterns#A28`, `principles#P28` · Enforcement: partial
...
*Origin: `BL-33` · Enforcement: partial (`ragcore/migrations/versions/0014_outbox_message.py`)*

### BL-34 — Transport / broker
**Azure Service Bus Standard.** See `.claude/rules/80-security-ops.md` §80.5; platform topology is
owned by **A2**.
*Origin: `BL-34` · Enforcement: procedural*
...
*Origin: `lang/dotnet#DN17` · satisfies `principles#P18`, `principles#P25` · Enforcement: partial —
...
*Origin: `lang/dotnet#DN22` · Enforcement: partial — SYSLIB1054 is informational by default; the
...
*Origin: `lang/dotnet#DN36` · Enforcement: mechanical — CA1416 is on by default as a **warning** for
...
*Origin: `lang/dotnet#DN1` · Enforcement: mechanical via
...
*Origin: `lang/dotnet#DN10` · core · satisfies `anti-patterns#A15`, `principles#P30` · Enforcement:
...
*Origin: `lang/dotnet#DN26` · core · satisfies `anti-patterns#A30`, `principles#P30` · Enforcement:
...
*Origin: `lang/dotnet#DN15` · satisfies `principles#P31` · Enforcement: partial — CA2000 is off by
...
*Origin: `lang/dotnet#DN16` · satisfies `secure-coding#SC2`, `principles#P11` · Enforcement:
...
> **Deviation DV-6, open (low severity).** The rule *is* gated today, so the concern is the
> mechanism, not the outcome: if `TreatWarningsAsErrors` were ever scoped down, DN-16 would
...
*Origin: `lang/dotnet#DN12` · satisfies `principles#P18` · Enforcement: partial — CA1051 off by
...
*Origin: `lang/dotnet#DN11` · satisfies `principles#P17`, `principles#P10` · Enforcement: partial —
...
*Origin: `lang/dotnet#DN20` · satisfies `principles#P25` · Enforcement: procedural — the source
...
*Origin: `lang/dotnet#DN18` · Enforcement: partial — CA1305, CA1307 and CA1310 are off by default
...
**document why**.
*Origin: `lang/dotnet#DN13` · Enforcement: **currently-unenforced** — the source's mechanism is a
...
*Origin: `lang/dotnet#DN33` · satisfies `performance#PF11` · Enforcement: partial — CA1859 is on by
...
*Origin: `lang/dotnet#DN37` · satisfies `principles#P7`, `principles#P8` · Enforcement:
...
exception.
*Origin: `lang/dotnet#DN19` · Enforcement: **currently-unenforced** — the source is explicit that no
...
*Origin: `lang/dotnet#DN25` · core · satisfies `secure-coding#SC22` · Enforcement: partial — CA5350
...
*Origin: `lang/dotnet#DN34` · core · satisfies `secure-coding#SC30` · Enforcement: partial — CA5359
...
*Origin: `lang/dotnet#DN35` · core · satisfies `secure-coding#SC30` · Enforcement: partial — CA5364
...
*Origin: `lang/dotnet#DN28`, `dotnet#P-DN-1` · Enforcement: mechanical —
...
*Origin: `lang/dotnet#DN29`, `dotnet#P-DN-1b` · Enforcement: mechanical —
...
*Origin: `lang/dotnet#DN21` · Enforcement: procedural — the source records that no in-box analyzer
...
> requires an ADR. Both the requirement and the gate are stated; neither substitutes for the other.
...
`.claude/rules/40-testing.md` §40.2. They are not duplicated here.
