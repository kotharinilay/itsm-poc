...
- [ ] T344 [P] Define the **production hosting tier for the two web portals** and set the CSP baseline there. The policy is built and unit-tested in `platform-core/security/content-security-policy.ts` and delivered for the desktop renderer by the Electron main process, but **no hosting tier in the repository sends it for the portals** — `index.html` defers it to a tier that does not exist yet, and `ng serve` cannot supply it because the baseline needs a per-response nonce (**T048**). Whatever the tier becomes must do three things: a fresh nonce per document, the same nonce on `ngCspNonce`, and the header from `buildCspHeader` rather than a restatement of it — Boundary: browser security | Validates: Constitution P-VII, Plan Stage 3
